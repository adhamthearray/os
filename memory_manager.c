 ...
 */
 ...
 void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
  {
 	...
 	// panic("allocateMem() is not implemented yet...!!");
 	 ...
 	 int h= pf_add_empty_env_page(e,(virtual_address+(i*PAGE_SIZE)),1);
 	 if(h!=0){
 		 FINAL_INDEX=i;
 		 NOT_E_SPACE=1;
 		 break;
 ...
 void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size) {
     int n_pages = ROUNDUP(size, PAGE_SIZE) / PAGE_SIZE;
cprintf("hahahahaha");
     for (int i = 0; i < n_pages; i++) {
         uint32 va = virtual_address + i * PAGE_SIZE;
         uint32 *pt = NULL;
         struct Frame_Info *frame = get_frame_info(e->env_page_directory, (void*)va, &pt);

         if (frame != NULL) {
             // --- Case 1: Buffered ---
             if (frame->isBuffered) {
                 if (pt && (pt[PTX(va)] & PERM_MODIFIED))
                     LIST_REMOVE(&modified_frame_list, frame);
                 else
                     LIST_REMOVE(&free_frame_list, frame);


                 if (pt) pt[PTX(va)] &= ~PERM_BUFFERED;
                 // Free the frame after all references are cleared
                 free_frame(frame);
         ...
         for (int ws = 0; ws < e->page_WS_max_size; ws++) {
             if (e->ptr_pageWorkingSet[ws].virtual_address == va) {
                 env_page_ws_clear_entry(e, ws);
                 //free_frame(frame); doctor 2alet ghalat
                 unmap_frame(e->env_page_directory,(void*)va);//correction,doctor's confirmation
                 pt_clear_page_table_entry(e, va);
                 break;
             }
         }
         ...
         if (pt!= NULL) {
             ...
             if (is_empty) {
            	 cprintf("haga hena etmasahet \n");



            	 cprintf("S");
                 unmap_frame(ptr_page_directory,(void*)pt);
                 //struct Frame_Info *pt_frame = get_frame_info(ptr_page_directory, (void*)pt, NULL);
